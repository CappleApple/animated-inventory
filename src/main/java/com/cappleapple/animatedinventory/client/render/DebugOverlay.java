...
public final class DebugOverlay {
    public static void render(GuiGraphics graphics, ClientRuntime runtime, long now) {
        ...
        try {
            ...
            for (var a : runtime.animations.active()) {
                ...
                graphics.drawString(Minecraft.getInstance().font, a.transition.type() + " " + a.transactionId, (int)a.source.x(), (int)a.source.y() - 8, 0xffffffff);
            }
            ...
            for (int i = 0; i < labels.length; i++) {
                ...
                graphics.drawString(font, labels[i], 5, 5 + i * 11, 0xffffffff);
            }
        } finally { graphics.pose().popPose(); }
    }
    private static void outline(GuiGraphics graphics, Bounds bounds, int color) {
        graphics.renderOutline((int)bounds.x(), (int)bounds.y(), (int)bounds.width(), (int)bounds.height(), color);
    }
