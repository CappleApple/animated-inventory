...
public final class ItemAnimation {
    ...
    public double progress(long now) { return com.cappleapple.animatedinventory.api.animation.Clamp.value((double)(now - start) / duration, 0, 1); }
    ...
    public double alpha(long now) {
        double edgeFade = (transition.type() == TransitionType.STOW || transition.type() == TransitionType.CRAFT) ? 1 - com.cappleapple.animatedinventory.api.animation.Clamp.value((progress(now) - .65) / .35, 0, 1) : 1;
        double arrivalFade = transition.type() == TransitionType.RETRIEVE ? com.cappleapple.animatedinventory.api.animation.Clamp.value(progress(now) / .25, 0, 1) : 1;
        return com.cappleapple.animatedinventory.api.animation.Clamp.value(channel(options.startAlpha(), options.endAlpha(), now), 0, 1) * edgeFade * arrivalFade;
    }
