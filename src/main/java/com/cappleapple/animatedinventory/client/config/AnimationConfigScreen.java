...
public final class AnimationConfigScreen extends Screen {
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.centeredText(font, "Page " + (page + 1) + " / " + ((settings.size() + rows() - 1) / rows()), width / 2, 26, 0xffaaaaaa);
        int row = 0;
        for (var entry : fields.entrySet()) {
            String key = entry.getKey().key(); int dot = key.indexOf('.');
            graphics.text(font, key.substring(dot + 1), width / 2 - 170, 42 + row * 32, 0xffffffff, false);
            graphics.text(font, key.substring(0, dot), width / 2 - 170, 52 + row * 32, 0xff999999, false);
            if (entry.getKey().get() instanceof Enum<?> option && entry.getValue().isHovered()) {
                ...
                graphics.setTooltipForNextFrame(font, Component.literal(allowed), mouseX, mouseY);
            }
            row++;
        }
        if (!error.isEmpty()) graphics.centeredText(font, error, width / 2, height - 48, 0xffff6666);
    }
}
