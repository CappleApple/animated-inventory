...
public final class AnimationConfigScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        for (int i = page * count; i < Math.min(settings.size(), (page + 1) * count); i++) {
            ...
            if (value.get() instanceof Boolean || value.get() instanceof Enum<?>) {
                control = Button.builder(valueLabel(value.get()), button -> {
                    if (!apply()) return;
    ...
    @Override public void onClose() { if (apply()) minecraft.gui.setScreen(parent); }
    @Override public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        super.extractRenderState(graphics, mouseX, mouseY, partialTick);
        graphics.centeredText(font, title, width / 2, 10, 0xffffffff);
        graphics.centeredText(font, Component.translatable(PREFIX + "ui.page", page + 1, (settings.size() + rows() - 1) / rows()), width / 2, 25, 0xffaaaaaa);
        int row = 0;
        for (var value : controls.keySet()) {
            int y = 42 + row * ROW_HEIGHT;
            var lines = font.split(label(value), Math.max(20, controlX() - left() - 10));
            for (int line = 0; line < Math.min(2, lines.size()); line++) graphics.text(font, lines.get(line), left(), y + line * 9, 0xffffffff, false);
            graphics.text(font, section(value), left(), y + 21, 0xffaaaaaa, false);
            if (mouseX >= left() && mouseX < controlX() && mouseY >= y && mouseY < y + ROW_HEIGHT) {
                graphics.setTooltipForNextFrame(font, description(value), mouseX, mouseY);
            }
            row++;
        }
        if (!error.getString().isEmpty()) graphics.centeredText(font, error, width / 2, height - 45, 0xffff6666);
    }
}
