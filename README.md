...
This branch builds **version 1.1.1 for Minecraft 26.2 NeoForge**. The original NeoForge 1.21.1 version remains on `main`.

## Installation

Minecraft **26.2**, **NeoForge 26.2.0.88**, and **Java 25**. Install `animatedinventory-neoforge-26.2-1.1.1.jar` in the client's `mods` directory. A server installation is not required.
...
Use **JDK 25** to run the build. The wrapper pins **Gradle 9.5.1** and the build uses **ModDevGradle 2.0.147**. The Java toolchain compiles for Java 25.

```powershell
.\gradlew.bat test build
```

The installable jar is `build/libs/animatedinventory-neoforge-26.2-1.1.1.jar`. Source jars are for development.
